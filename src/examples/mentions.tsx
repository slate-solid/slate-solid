...
const MentionExample = () => {
  ...
  const [target, setTarget] = createSignal<Range | null>(null)
  const [index, setIndex] = createSignal(0)
  const [search, setSearch] = createSignal('')
  const renderElement = (props: RenderElementProps) => <Element {...props} />
  const renderLeaf = (props: RenderLeafProps) => <Leaf {...props} />
  const editor = createMemo(() =>
    withMentions(withSolid(withHistory(createEditor()))),
  )
    ...
    if (t && c.length > 0) {
      switch (event.key) {
        case 'ArrowDown': {
          ...
          const prevIndex = index() >= c.length - 1 ? 0 : index() + 1
          setIndex(prevIndex)
          break
        }
        case 'ArrowUp': {
          event.preventDefault()
          const nextIndex = index() <= 0 ? c.length - 1 : index() - 1
          setIndex(nextIndex)
          break
        }
        case 'Tab':
  ...
  createEffect(() => {
    ...
    if (t && c.length > 0) {
      ...
      const domRange = SolidEditor.toDOMRange(editor(), t)
...
const Element = (props: RenderElementProps) => {
  return (
    <Switch fallback={<p {...props.attributes}>{props.children}</p>}>
      <Match when={props.element.type === 'mention'}>
        <Mention
          {...(props as RenderElementProps & { element: MentionElement })}
        />
      </Match>
    </Switch>
  )
...
const Mention = (props: RenderElementProps & { element: MentionElement }) => {
  ...
  const style = () => {
    ...
    if ('bold' in props.element.children[0] && props.element.children[0].bold) {
      style['font-weight'] = 'bold'
    }
    if (
      'italic' in props.element.children[0] &&
      props.element.children[0].italic
    ) {
